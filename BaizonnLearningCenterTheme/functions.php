...
function theme_scripts() {
    ...
    wp_localize_script('theme-script', 'screenReaderText', array(
        'expand' => __('expand child menu', 'website4729546'),
        'collapse' => __('collapse child menu', 'website4729546'),
    ));
}
...
function theme_widgets_init2() {
    register_sidebar(array(
        'name'          => __('Primary Widget Area', 'website4729546'),
        'id'            => 'primary',
        'description'   => __('Add widgets here to appear in your sidebar on blog posts and archive pages.', 'website4729546'),
...
function _theme_get_excerpt($args = array()) {
	...
	$more_tag                     = isset($args['more_tag'])                     ? $args['more_tag']                     : __('Read more', 'website4729546');
...
function theme_get_category_list() {
	return str_replace(
		'<a',
		'<a class="u-textlink"',
		get_the_category_list(_x(', ', 'Used between list items, there is a space after the comma.', 'website4729546'))
	);
}

function theme_get_tags_list() {
    return get_the_tag_list('', _x( ', ', 'Used between list items, there is a space after the comma.', 'website4729546'));
}
...
function getProductButtonHtml ($button_html, $product, $clickTypeProductbutton = 'add-to-cart', $contentProductbutton = '') {
    ...
    $buttonText = sprintf(__('Add to cart', 'website4729546'));
    if ($contentProductbutton !== '') {
        $buttonText =  sprintf(__($contentProductbutton, 'website4729546'));
    }
...
function theme_404_content($args = '') {
    $args = wp_parse_args($args, array(
        'error_title' => __('Nothing here', 'website4729546'),
        'error_message' => __('It looks like nothing was found at this location. Maybe try a search?', 'website4729546')
    ));
    extract($args);
...
function get_categories_html($args) {
    ...
    if ($category_id) {
        if (is_category()) {
            ...
            if ($category->parent) {
                // parent cat url
                $linkUrl = esc_url(get_category_link($category->parent));
            } else {
                //blog url
                $linkUrl = get_option('page_for_posts') ? get_permalink(get_option('page_for_posts')) : get_home_url();
            }
            $categories_html .= str_replace(array('{content}', '{url}', '{activeLi}', '{activeLink}'), array('&#10094; &nbsp;' . __( 'Back' ), $linkUrl, '', ''), $args['itemTemplate']);
        }
        // add current cat link
        $categories_html .= str_replace(array('{content}', '{url}', '{activeLi}', '{activeLink}'), array(esc_attr($category->name), esc_url(get_term_link($category)), 'u-active', 'active'), $args['itemTemplate']);
    }
    ...
    if($sub_cats) {
        foreach($sub_cats as $sub_category) {
            $categories_html .= str_replace(array('{content}', '{url}', '{activeLi}', '{activeLink}'), array(esc_attr($sub_category->name), esc_url(get_term_link($sub_category)), '', ''), $args['itemTemplate']);
        }
    }
